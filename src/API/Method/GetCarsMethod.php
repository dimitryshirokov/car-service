 ...
 */
class GetCarsMethod implements Procedure
{
    /**
    ...
    public function __invoke(Request $request): Response
    {
        ...
        try {
            $result = $this->getCarService()->getAll($params['limit'], $params['offset']);
        } catch (Throwable $exception) {
            return new Error(-32603, $exception->getMessage(), $exception->__toString(), $request->id());
        }

        return new Success($request->id(), ['data' => $result]);
    }
